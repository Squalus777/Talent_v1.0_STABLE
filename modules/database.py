...
# --- HASH FUNKCIJA (ISTI STANDARD SVUGDJE) ---
def get_hash(text):
...
def init_db():
    ...
    # ---------------------------------------------------------
    # 🔥 AUTOMATSKA INICIJALIZACIJA (SIGURNA) 🔥
    # ---------------------------------------------------------
    
    # 1. Osiguraj Admina (uvijek mora postojati)
    admin_hash = get_hash("admin123")
    # INSERT OR IGNORE: Ako admin postoji (i možda ima drugu lozinku), NE DIRAJ GA.
    # Ako ne postoji, kreiraj ga s admin123.
    # Ako želite prisilni reset admina svaki put, koristite REPLACE, ali za sada je IGNORE sigurniji za vas.
    ...
    # 2. "Pometi" sve zaposlenike koji nemaju login (SIGURNO KREIRANJE)
    default_hash = get_hash("lozinka123")
    
    # Ova SQL naredba umeće u USERS samo one iz EMPLOYEES_MASTER koji tamo već nisu.
    # Ne dira postojeće korisnike niti njihove lozinke.
    c.execute("""
