...
def render_manager_view():
    ...
    if menu == "📊 Dashboard":
        ...
        t1, t2 = st.tabs(["9-Box Matrica", "Povijest"])
        with t1:
            if not evals.empty:
                fig = px.scatter(evals, x="avg_performance", y="avg_potential", color="category", text="ime_prezime", 
                                 range_x=[0.5,5.5], range_y=[0.5,5.5], title="9-Box Matrica")
    ...
    # ----------------------------------------------------------------
    # 3. CILJEVI TIMA (POPRAVLJENA LOGIKA I SIGURNOST)
    # ----------------------------------------------------------------
        ...
        # PRIKAZ CILJEVA PO ZAPOSLENICIMA
        for _, emp in my_team.iterrows():
            ...
            # Provjera ukupne težine ciljeva (Mora biti 100%)
            ...
            with st.expander(f"👤 {emp['ime_prezime']} (Ukupna težina ciljeva: :{color}[{tot_w}%])"):
                ...
                for _, g in goals.iterrows():
                    ...
                    # FORMA ZA UREĐIVANJE SAMOG CILJA
                    ...
                    # LOGIKA ZA SPREMANJE KPI-a
                    if st.button("💾 Spremi KPI i Izračunaj", key=f"s_{gid}"):
                        # 1. Provjera zbroja težina KPI-eva
                        ...
                        if current_kpi_sum != 100:
                            ...
                            # 2. Brisanje starih i unos novih
                            conn.execute("DELETE FROM goal_kpis WHERE goal_id=?", (gid,))
                            ...
                            for _, r in ed.iterrows():
                                if str(r['KPI Naziv']).strip():
                                    ...
                                    conn.execute("INSERT INTO goal_kpis (goal_id, description, weight, progress) VALUES (?,?,?,?)", 
                                                 (gid, str(r['KPI Naziv']), w_val, p_val))
                                    
                                    # Računanje napretka cilja: (KPI Težina * KPI Ostvarenje) / 100
                                    weighted_progress_sum += (w_val * p_val) / 100
                            
                            # 3. Ažuriranje glavnog cilja
                            conn.execute("UPDATE goals SET progress=?, last_updated=? WHERE id=?", 
                                         (weighted_progress_sum, datetime.now().strftime("%Y-%m-%d"), gid))
                            conn.commit()
                            st.success(f"✅ Spremljeno! Napredak cilja je sada: {weighted_progress_sum:.1f}%")
                            time.sleep(1); st.rerun()
                    
                    # Prikaz progress bara za cilj
                    ...
                    if is_locked:
                        ...
                        if st.button("🖨️ Prikaži PDF Preview", key=f"pdf_{kid}"):
                            ...
                            <div style="border:2px solid #ccc; padding:20px; font-family:Arial;">
                                ...
                                <hr>
                                <p><i>Dokument generiran sustavom Tommy Talent</i></p>
                            </div>
                            """
    ...
    # ----------------------------------------------------------------
    # 5. IDP (POPRAVLJENO SPREMANJE I TROŠAK)
    # ----------------------------------------------------------------
                ...
                if st.button("💾 SPREMI IDP", key=f"b_idp_{eid}"):
                    try:
                        ...
                        with sqlite3.connect(DB_FILE) as db:
                            # FIX ZA SPREMANJE: Prisilna konverzija u string prije JSON-a
