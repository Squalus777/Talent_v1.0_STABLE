...
def render_hr_view():
    ...
    if menu == "📊 HR Dashboard":
        ...
        if not f_ev.empty:
            fig = px.scatter(f_ev, x="avg_performance", y="avg_potential", color="Tip",
                             ...
                             range_x=[0, 5.5], range_y=[0, 5.5], title="9-Box")
    ...
    # --- KLJUČNI DIO: AUTOMATSKO KREIRANJE KORISNIKA KOD IMPORTA/UNOSA ---
                ...
                if st.form_submit_button("Spremi"):
                    if kb and ip:
                        ...
                        with sqlite3.connect(DB_FILE) as db:
                            ...
                            # AUTOMATSKI INSERT OR IGNORE - ako postoji, ne dira lozinku. Ako ne, kreira.
                ...
                try:
                    ...
                    with sqlite3.connect(DB_FILE) as db:
                        ...
                        for _, r in df_i.iterrows():
                            ...
                            # AUTOMATSKI INSERT OR IGNORE - Sigurno za postojeće korisnike
                            ...
                            # Ažuriraj rolu i odjel u users tablici (ali ne lozinku!)
        ...
        with t1:
            per_list = pd.read_sql_query("SELECT period_name FROM periods WHERE company_id=?", conn, params=(company_id,))
            s_p = st.selectbox("Aktivno:", per_list['period_name'].tolist() if not per_list.empty else [])
            if st.button("Postavi"):
                conn.execute("UPDATE app_settings SET setting_value=? WHERE setting_key='active_period'", (s_p,))
                conn.commit(); st.rerun()
            st.divider()
            with st.form("np"):
                nn = st.text_input("Novi naziv"); nd = st.date_input("Rok")
                if st.form_submit_button("Kreiraj"):
                    conn.execute("INSERT OR REPLACE INTO periods (period_name, deadline, company_id) VALUES (?,?,?)", (nn, str(nd), company_id))
                    conn.execute("UPDATE app_settings SET setting_value=? WHERE setting_key='active_period'", (nn,))
                    conn.commit(); st.rerun()
        with t2:
            st.write(f"Trenutni rok: {deadline}")
            nr = st.date_input("Novi rok")
            if st.button("Spremi rok"):
                conn.execute("UPDATE periods SET deadline=? WHERE period_name=?", (str(nr), current_period))
                conn.commit(); st.rerun()
        with t3:
            if not per_list.empty:
                dp = st.selectbox("Briši:", per_list['period_name'].tolist())
                if st.button("Obriši") and dp != current_period:
                    conn.execute("DELETE FROM periods WHERE period_name=?", (dp,))
                    conn.commit(); st.rerun()
